<template>
  <MainLayout>
    <div id="AddressPage" class="mt-4 max-w-[500px] mx-auto px-2">
      <div class="mx-auto bg-[#CCDAE0] rounded-lg p-3 text-[#0E2167]">
        ...
        <form @submit.prevent="submit()">
          ...
          <TextInput
            ...
            inputType="text"
            :error="error && error.type == 'postCode' ? error.message : ''"
          />
          ...
          <button
            :disabled="isWorking"
            type="submit"
            class="mt-6 bg-gradient-to-r from-[#0E2167] to-[#23AAEF] w-full text-white text-[21px] font-semibold p-1.5 rounded-full"
          >
...
const submit = async () => {
  ...
  if (!contactName.value) {
    ...
    error.value = {
      type: "zipCode",
      message: "A psot code is required",
    };
  } else if (!city.value) {
    error.value = {
      type: "city",
